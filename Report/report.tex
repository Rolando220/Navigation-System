...
\chapter{Decoupled INS/GNSS and AHRS Integration via Error-State KF}
...
\section{The Error-State KF Formulation}
...
\begin{equation}
    \delta x_{att} = \begin{bmatrix} \delta\phi, \delta\theta, \delta\psi, \delta b_{gx}, \delta b_{gy}, \delta b_{gz} \end{bmatrix}^T = \begin{bmatrix} \delta\Theta \\ \delta b_g \end{bmatrix}
\end{equation}
...
\begin{equation}
    \delta\dot{\Theta} = -J \delta b_g - J w_g
\end{equation}
...
\begin{equation}
    \delta x_{nav} = \begin{bmatrix} \delta\varphi, \delta\lambda, \delta h, \delta v_N, \delta v_E, \delta v_D \end{bmatrix}^T = \begin{bmatrix} \delta P \\ \delta V^n \end{bmatrix}
\end{equation}
...
\end{equation}
\begin{equation}
    T_r = \begin{bmatrix} \frac{1}{R_M+h} & 0 & 0 \\ 0 & \frac{1}{(R_N+h)\cos\varphi} & 0 \\ 0 & 0 & -1 \end{bmatrix}
\end{equation}
...
\chapter{Simulation Results and Performance Analysis}
...
\section{Attitude and Bias Estimation Performance}
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/error_att_NO_filt.png}}
    \caption{Time evolution of the attitude estimation error resulting from pure mechanization, with no filter applied.}
    \label{fig:error_att_NO_filt}
\end{figure}

The introduction of the Error-State Kalman Filter, aided by the AHRS measurements at $100\,\mathrm{Hz}$, effectively bounds these fluctuations. As shown in Figure \ref{fig:error_att_filt}, the ESKF maintains the attitude estimation tightly coupled to the ground-truth kinematics, completely suppressing the drift and reducing the error to zero-mean noise typical of a well-tuned filter.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/att_error_filt.png}}
    \caption{Time evolution of the attitude estimation error resulting from pure mechanization, with the ESKF filter applied.}
    \label{fig:error_att_filt}
\end{figure}

To quantitatively evaluate the filter's performance, the statistical metrics are compared in Table \ref{tab:attitude_comparison}. The table highlights the massive improvement introduced by the ESKF. While the pure INS accumulates large absolute errors and an RMS yaw error of over $8.18\,\mathrm{rad}$ during the flight, the closed-loop ESKF drastically reduces these values to the order of $10^{-3}\,\mathrm{rad}$. Furthermore, the ESKF achieves a mean error of exactly zero across all axes, proving that the estimation is perfectly unbiased and operates well within the nominal accuracy of the selected AHRS module.

\begin{table}[htbp]
    \centering
    \small
    \caption{Statistical metrics comparison for attitude estimation (Pure Mechanization vs. ESKF).}
    \label{tab:attitude_comparison}
    \begin{tabularx}{\textwidth}{l l c c c}
        \toprule
        \textbf{Configuration} & \textbf{Statistic} & \textbf{Roll [rad]} & \textbf{Pitch [rad]} & \textbf{Yaw [rad]} \\
        \midrule
        \textbf{Pure Mech.} & Max Abs Error & 2.1643 & 3.2689 & 13.8673 \\
        \textbf{Pure Mech.} & Mean Error & 0.4080 & 0.2439 & -7.1589 \\
        \textbf{Pure Mech.} & RMS Error & 0.8369 & 0.9440 & 8.1845 \\
        \midrule
        \textbf{ESKF} & Max Abs Error & 0.0165 & 0.0136 & 0.0213 \\
        \textbf{ESKF} & Mean Error & 0.0000 & 0.0000 & 0.0000 \\
        \textbf{ESKF} & RMS Error & 0.0030 & 0.0030 & 0.0042 \\
        \bottomrule
    \end{tabularx}
\end{table}

A critical aspect of the filter's success is its ability to dynamically estimate the deterministic sensor errors. The in-run gyroscope biases, which were initialized at zero in the filter state, rapidly converge to the true constant values injected into the simulation environment. This validates the system's compliance with the \textbf{G/a} specification, ensuring that the mechanization step is fed with continuously corrected angular rates.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/est_bias.png}}
    \caption{Time evolution of the gyroscope biases estimated by the ESKF.}
    \label{fig:gyro_bias_convergence}
\end{figure}

The statistical evaluation of the bias estimation is summarized in Table \ref{tab:gyro_bias_stats}. Expressing the rates in the body frame ($p, q, r$), the results show that once the initial transient has settled, the mean values of the estimated biases perfectly align with the nominal uncalibrated offsets defined in the sensor model ($0.005$, $-0.003$, and $0.006\,\mathrm{rad/s}$ respectively). This excellent tracking confirms the robust observability and optimal tuning of the attitude filter.

\begin{table}[htbp]
    \centering
    \small
    \caption{Statistical metrics for the estimated gyroscope biases (ESKF).}
    \label{tab:gyro_bias_stats}
    \begin{tabularx}{\textwidth}{l c c c}
        \toprule
        \textbf{Statistic} & \textbf{Bias $p$ [rad/s]} & \textbf{Bias $q$ [rad/s]} & \textbf{Bias $r$ [rad/s]} \\
        \midrule
        \textbf{Max Abs Value} & 0.0909 & 0.0526 & 0.0583 \\
        \textbf{Mean Value} & 0.0050 & -0.0030 & 0.0060 \\
        \textbf{RMS Value} & 0.0050 & 0.0030 & 0.0060 \\
        \bottomrule
    \end{tabularx}
\end{table}

To further visualize the filter's noise attenuation capabilities, the actual state trajectories are compared against the noisy measurements and the filtered estimates. Figures \ref{fig:traj_roll}, \ref{fig:traj_pitch}, and \ref{fig:traj_yaw} display a highly zoomed-in time window of the attitude tracking. The raw AHRS measurements are visibly corrupted by the high-frequency stochastic noise injected into the simulation. In contrast, the ESKF solution seamlessly tracks the ground-truth dynamics, effectively smoothing the measurement noise without introducing any noticeable phase lag.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/roll.png}}
    \caption{Roll trajectory tracking (zoomed): Comparison between ground-truth, noisy AHRS measurement, and the smoothed ESKF estimation.}
    \label{fig:traj_roll}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/pitch.png}}
    \caption{Pitch trajectory tracking (zoomed): Comparison between ground-truth, noisy AHRS measurement, and the smoothed ESKF estimation.}
    \label{fig:traj_pitch}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/yaw.png}}
    \caption{Yaw trajectory tracking (zoomed): Comparison between ground-truth, noisy AHRS measurement, and the smoothed ESKF estimation.}
    \label{fig:traj_yaw}
\end{figure}

To explicitly quantify this noise attenuation, the measurement errors (pure sensor noise) are plotted directly against the filter's estimation errors in Figures \ref{fig:err_comp_roll}, \ref{fig:err_comp_pitch}, and \ref{fig:err_comp_yaw}. The high-frequency stochastic noise introduced by the AHRS is visibly bounded and reduced by the ESKF. These plots demonstrate the filter's capability to effectively filter out the measurement scatter, yielding a clean attitude solution that mitigates the intrinsic inaccuracies of the single sensors.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/roll_err.png}}
    \caption{Roll error comparison (zoomed): AHRS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_roll}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/pitch_err.png}}
    \caption{Pitch error comparison (zoomed): AHRS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_pitch}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/yaw_err.png}}
    \caption{Yaw error comparison (zoomed): AHRS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_yaw}
\end{figure}

\clearpage

\section{Position and Velocity Estimation Performance}
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/pos_error_NO_filt.png}}
    \caption{Time evolution of the position estimation error resulting from pure mechanization, with no filter applied.}
    \label{fig:error_pos_NO_filt}
\end{figure}

As detailed in Table \ref{tab:pos_comparison} and Figure \ref{fig:error_pos_NO_filt}, the pure mechanization completely loses track of the vehicle. By the end of the simulation, it accumulates altitude errors in the order of thousands of kilometers (RMS of $\sim 3 \times 10^6\,\mathrm{m}$) and horizontal geodetic errors exceeding 1 radian, rendering the pure INS solution totally unusable.

The GNSS-aided ESKF, operating asynchronously to process the $10\,\mathrm{Hz}$ measurements, drastically corrects this behavior. Figures \ref{fig:error_lat_lon_filt} and \ref{fig:error_alt_filt} illustrate the stable, bounded estimation errors provided by the closed-loop architecture.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/lat_lon_filt_err.png}}
    \caption{Time evolution of the latitude/longitude estimation error resulting from pure mechanization, with ESKF applied.}
    \label{fig:error_lat_lon_filt}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/alt_filt_err.png}}
    \caption{Time evolution of the altitude estimation error resulting from pure mechanization, with ESKF applied.}
    \label{fig:error_alt_filt}
\end{figure}

The filter successfully collapses the horizontal position error down to the order of $10^{-7}$ radians (equivalent to a few meters physically), achieving a strictly zero-mean error. Furthermore, the vertical channel, notoriously the most susceptible to drift, remains perfectly bounded. The ESKF achieves a steady-state altitude RMS error of just $0.1658\,\mathrm{m}$, exceptionally outperforming the conservative $3.5\,\mathrm{m}$ standard deviation expected from the GNSS receiver. 

\begin{table}[htbp]
    \centering
    \small
    \caption{Statistical metrics comparison for LLH estimation (Pure Mechanization vs. ESKF).}
    \label{tab:pos_comparison}
    \begin{tabularx}{\textwidth}{l l c c c}
        \toprule
        \textbf{Configuration} & \textbf{Statistic} & \textbf{Latitude [rad]} & \textbf{Longitude [rad]} & \textbf{Altitude [m]} \\
        \midrule
        \textbf{Pure Mech.} & Max Abs Error & 4.3110 & 1.2696 & $\sim 7 \times 10^6$ \\
        \textbf{Pure Mech.} & Mean Error & 0.6111 & -0.4004 & $\sim 2 \times 10^6$ \\
        \textbf{Pure Mech.} & RMS Error & 1.0283 & 0.5003 & $\sim 3 \times 10^6$ \\
        \midrule
        \textbf{ESKF} & Max Abs Error & $\sim 2 \times 10^{-7}$ & $\sim 5 \times 10^{-7}$ & 0.4301 \\
        \textbf{ESKF} & Mean Error & 0.0000 & 0.0000 & -0.0240 \\
        \textbf{ESKF} & RMS Error & 0.0000 & 0.0000 & 0.1658 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_error_NO_filt.png}}
    \caption{Time evolution of the velocity estimation error resulting from pure mechanization, with no filter applied.}
    \label{fig:error_vel_NO_filt}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_error_filt.png}}
    \caption{Time evolution of the velocity estimation error resulting from pure mechanization, with ESKF applied.}
    \label{fig:error_vel_filt}
\end{figure}

The asynchronous correction logic efficiently handles the $10\,\mathrm{Hz}$ update rate, preventing the high-frequency IMU noise from degrading the low-frequency stability provided by the GNSS. As detailed in Table \ref{tab:vel_comparison}, the filtered velocity errors maintain an RMS below $0.018\,\mathrm{m/s}$ across all axes, which is substantially better than the $0.05\,\mathrm{m/s}$ nominal accuracy of the GNSS module. The mean velocity errors are strongly centered around zero, demonstrating the robustness of the decoupled architecture even during a prolonged stratospheric flight.

\begin{table}[htbp]
    \centering
    \small
    \caption{Statistical metrics comparison for velocity estimation (Pure Mechanization vs. ESKF).}
    \label{tab:vel_comparison}
    \begin{tabularx}{\textwidth}{l l c c c}
        \toprule
        \textbf{Configuration} & \textbf{Statistic} & \textbf{$v_N$ [m/s]} & \textbf{$v_E$ [m/s]} & \textbf{$v_D$ [m/s]} \\
        \midrule
        \textbf{Pure Mech.} & Max Abs Error & $\sim 5672$ & $\sim 3443$ & $\sim 8214$ \\
        \textbf{Pure Mech.} & Mean Error & $\sim 2984$ & $\sim 1636$ & $-3986$ \\
        \textbf{Pure Mech.} & RMS Error & $\sim 3530$ & $\sim 2024$ & $\sim 4666$ \\
        \midrule
        \textbf{ESKF} & Max Abs Error & 0.0668 & 0.0707 & 0.0387 \\
        \textbf{ESKF} & Mean Error & -0.0017 & 0.0006 & -0.0005 \\
        \textbf{ESKF} & RMS Error & 0.0180 & 0.0179 & 0.0100 \\
        \bottomrule
    \end{tabularx}
\end{table}

The noise rejection performance is equally evident when analyzing the kinematic trajectories. Combining GPS and INS has the primary goal of limiting the estimation drift while simultaneously improving the absolute navigation solution. Figures \ref{fig:traj_vel_n}, \ref{fig:traj_vel_e}, and \ref{fig:traj_vel_d} show a highly zoomed-in perspective of the velocity components. The raw GNSS measurements exhibit discrete, low-rate ($10\,\mathrm{Hz}$) noisy updates. The closed-loop ESKF optimally fuses this information with the high-rate ($100\,\mathrm{Hz}$) inertial integration, yielding a smooth velocity estimate that closely follows the true trajectory, filtering out the GNSS scatter.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_n.png}}
    \caption{North velocity trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_vel_n}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_e.png}}
    \caption{East velocity trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_vel_e}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_d.png}}
    \caption{Down velocity trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_vel_d}
\end{figure}

In parallel, the position tracking performance is depicted in Figures \ref{fig:traj_lat}, \ref{fig:traj_lon}, and \ref{fig:traj_alt}. Despite the significant variance characterizing the standalone GNSS positional fixes, the integrated system effectively rejects the measurement noise. The estimated latitude, longitude, and altitude remain tightly anchored to the ground-truth path, visually confirming the superior accuracy and reliability of the integrated navigation system over uncoupled sensors.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/latitude.png}}
    \caption{Latitude trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_lat}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/longitude.png}}
    \caption{Longitude trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_lon}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/altitude.png}}
    \caption{Altitude trajectory tracking (zoomed): Comparison between ground-truth, noisy GNSS measurement, and the ESKF estimation.}
    \label{fig:traj_alt}
\end{figure}

\clearpage

Finally, the robustness of the decoupled architecture is validated by directly comparing the raw GNSS measurement errors against the ESKF estimation errors. Figures \ref{fig:err_comp_vel_n}, \ref{fig:err_comp_vel_e}, and \ref{fig:err_comp_vel_d} illustrate the velocity error attenuation, where the discrete measurement scatter is significantly damped by the filtering process. 

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_n_err.png}}
    \caption{North velocity error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_vel_n}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_e_err.png}}
    \caption{East velocity error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_vel_e}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/vel_d_err.png}}
    \caption{Down velocity error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_vel_d}
\end{figure}

Correspondingly, Figures \ref{fig:err_comp_lat}, \ref{fig:err_comp_lon}, and \ref{fig:err_comp_alt} demonstrate the position error reduction. The ESKF effectively bounds the GNSS positional inaccuracies, providing an estimation error that is continuously smoothed and kept well within the raw sensor's uncertainty limits, further proving the success of the data fusion algorithm.

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/lat_err.png}}
    \caption{Latitude error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_lat}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/lon_err.png}}
    \caption{Longitude error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_lon}
\end{figure}

\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{Images/alt_err.png}}
    \caption{Altitude error comparison (zoomed): GNSS measurement error vs. ESKF estimation error.}
    \label{fig:err_comp_alt}
\end{figure}
